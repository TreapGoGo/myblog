...
指数分布的无记忆性：对于任意的 $s,t>0$ ，有 $P(X>s+t|X>s)=P(X>t)$ 。
...
$$\displaystyle \varphi(x)=\frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{(x-\mu)^2}{2\sigma^2}},-\infty<x<+\infty$$

关于 $x=\mu$ 对称，在 $x=\mu$ 去最大值，在 $\mu\plusmn\sigma$ 处是拐点。

分布函数：

$$F(x)=\frac{1}{\sqrt{2\pi}\sigma}\int_{-\infty}^x e^{-\frac{(y-\mu)^2}{2\sigma^2}} \mathrm{d} y, -\infty<y<+\infty$$

标准正态分布： $X\sim N(0,1), \mu=0, \sigma=1$ 
