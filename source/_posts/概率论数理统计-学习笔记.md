...
| $A$ 与 $B$ 相等 |             $A=B$              |  事件 $A$ 发生必有事件 $A$ 发生，且事件 $B$ 发生必有事件 $A$ 发生  |           $A$ 与 $B$ 包含的样本点相同            |
...
$$P(B_i\vert A)=\frac{P(B_iA)}{P(A)}=\frac{P(A\vert B_i)P(B_i)}{\sum_{j=1}^n P(A\vert B_j)P(B_j)}$$
...
| 分布函数 | $F(x)=\left\{\begin{aligned}& 0, && x\le a \\& \frac{x-a}{b-a}, && a<x<b \\& 1, && x\ge b\end{aligned}\right.$ |   $F(x)=\left\{\begin{aligned} & 1-\mathrm{e}^{-\lambda x}, && x>0 \\ & 0, && 其他\end{aligned}\right.$   | $F(x)=\frac{1}{\sqrt{2\pi}\sigma}\int_{-\infty}^x \mathrm{e}^{-\frac{(y-\mu)^2}{2\sigma^2}} \mathrm{d} y, -\infty<y<+\infty$ |
