...
> Q：什么叫“随机变量函数”？
> 
> A：其实就是随机变量套随机变量。更具体地，即一个随机变量 $Y$ 关于另一个随机变量 $X$ 的变化而变化，将这种关系抽象为一种函数关系，例如 $Y=2X+8$ 。这里的 $X$ 和 $Y$ 不是具体的实数，而是随机变量，其具有的属性也不是一个数值，而包含了定义域（样本空间）、分布律（或概率密度）、分布函数等多个属性。
>
> Q：什么叫“随机变量函数的分布”？
>
> A：其实就是在上述的那种函数关系中，我们更加关注的是，从其中一个随机变量的概率密度/分布函数/分布律出发，如何得出另一个随机变量的概率密度/分布函数/分布律。
